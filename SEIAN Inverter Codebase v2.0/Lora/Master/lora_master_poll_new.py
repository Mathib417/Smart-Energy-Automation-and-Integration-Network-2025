import sys
sys.path.append('/home/Mathi.b_417/myenv/LoRa/pySX127x')  # Update if needed
...
class LoRaMaster(LoRa):
    ...
    def set_sync_word(self, sync_word):
        self.set_register(0x39, sync_word)
        time.sleep(0.1)
    ...
    def poll_inverter(self, inv_id, sync_word):
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            if resp:
                ...
                if rx_id == inv_id:
                    print(f"?? Received from {inv_id}: {data}")
                    self.send_ack(inv_id)
                    return data
                else:
                    print(f"?? Mismatched ID: got {rx_id}, expected {inv_id}")
            else:
                print(f"?? No response from {inv_id}")
